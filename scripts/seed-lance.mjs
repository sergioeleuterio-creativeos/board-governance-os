...
function brainEntries(organization, company, user, sourceDocumentId) {
  const base = {
    organization_id: organization.id,
    company_id: company.id,
  ...
  return [
    {
      ...
      risk_score: 72,
      confidence_score: 68,
      perspective: 'A tese de owned audience e correta, mas o board nao deveria aprovar investimento incremental sem DRE por produto, OCF, margem por formato e concentracao por anunciante/categoria.',
      strategic_questions: [
        'Qual receita e margem por display, video, branded content, creators, app, CRM, assinaturas e eventos?',
        'Qual o payback esperado para o investimento no app e em dados first-party?',
      ...
      recommendations: [
        ...
        { title: 'Definir investment gate do app', detail: 'Aprovar investimento por milestones de usuario identificado e receita incremental.', priority: 'high' },
      ],
    ...
    },
    {
      ...
      risk_score: 64,
      confidence_score: 76,
      perspective: 'O ecossistema tem muitas frentes simultaneas. A prioridade operacional e instalar uma cadencia semanal que una editorial, produto, tecnologia, dados e comercial.',
      strategic_questions: [
        ...
        'Quais tres KPIs fecham a semana como criterios de decisao?',
      ],
      recommendations: [
        { title: 'War room de 13 semanas', detail: 'Reuniao semanal com KPI tree do app, CRM, inventario e vendas.', priority: 'urgent' },
    ...
    },
    {
      ...
      risk_score: 58,
      confidence_score: 78,
      perspective: 'LANCE! ja tem massa critica. O maior upside esta em transformar alcance em segmentos acionaveis por clube, competicao, comportamento e propensao comercial.',
      strategic_questions: [
        'Quais segmentos de torcedor geram maior receita por usuario?',
      ...
      recommendations: [
        { title: 'Segmentos comercializaveis', detail: 'Montar pacotes por clubes, competicoes, jornada e intencao.', priority: 'high' },
    ...
    },
    {
      ...
      risk_score: 60,
      confidence_score: 77,
      perspective: 'A marca tem uma vantagem rara: relacao emocional com torcedores. O desafio e nao tratar usuario como inventario; o app precisa dar utilidade recorrente e personalizacao real.',
      strategic_questions: [
        'Qual promessa do app faz o torcedor voltar todos os dias?',
        'Como clubes, idols e competicoes personalizam a experiencia?',
        'Como medir valor de marca alem de views e impressions?',
      ],
      recommendations: [
        { title: 'Proposta de valor do torcedor', detail: 'Testar livescore, alertas, comunidades, beneficios e conteudo personalizado.', priority: 'high' },
...
async function seedGovernanceChain(organization, company, user, sourceDocumentId) {
  const diagnosis = [
    ...
    'A questao de board nao e gerar mais audiencia; e transformar alcance em relacionamento proprietario, dados first-party, receita de qualidade e cadencia de decisao entre editorial, produto, tecnologia e comercial.',
  ...
  const businessPlan = await must('insert business plan', service
    ...
    .insert({
      ...
      risks,
      assumptions: [
        { title: 'Fonte primaria', detail: 'Diagnostico baseado no Midia Kit oficial LANCE! 2026, sem acesso a DRE/OCF internos.' },
  ...
  const boardPack = await must('insert board pack', service
    ...
    .insert({
      ...
      meeting_agenda: [
        'Confirmar tese: alcance alugado vs audiencia proprietaria.',
  ...
  const boardSession = await must('insert board session', service
    ...
    .insert({
      ...
      closure_recommendation: 'commit_with_conditions',
      closure_summary: 'Prosseguir com LANCE! App/CRM como eixo estrategico, condicionado a DRE gerencial, gates de retencao/dados, guardrails reputacionais e cadencia semanal de execucao.',
  ...
  const reviews = [
    {
      ...
      risk_score: 70,
      confidence_score: 79,
      perspective: 'A recomendacao sintetizada e comprometer com a tese de owned audience, mas condicionar qualquer aceleracao a evidencias financeiras, dados first-party, guardrails e cadencia de decisao.',
      strategic_questions: [
      ...
      recommendations: [
        { title: 'Commit with conditions', detail: 'Aprovar direcao estrategica e travar gates de dados, caixa, receita e risco.', priority: 'urgent' },
  ...
  await must('insert agent conversations', service.from('agent_conversations').insert([
    {
      ...
      summary: 'Growth apoia aceleracao; Risk exige controles antes de escalar monetizacao.',
      conflicts: ['Velocidade comercial vs guardrails de reputacao e dados.'],
      agreements: ['Ambos concordam que owned audience e o eixo estrategico.'],
    },
    {
      organization_id: organization.id,
      company_id: company.id,
  ...
  const boardMeeting = await must('insert board meeting', service
    ...
    .insert({
      ...
      orchestrator_summary: 'Reuniao preparada para decidir commit with conditions.',
    })
    .select('id')
    .single())
  ...
  const decisions = await must('insert decisions', service.from('decisions').insert(decisionCandidates.map((decision, index) => ({
    ...
    owner_label: index === 0 ? 'Produto + Dados' : index === 1 ? 'Financeiro + CRO' : 'Editorial + Juridico',
    owner: index === 0 ? 'Produto + Dados' : index === 1 ? 'Financeiro + CRO' : 'Editorial + Juridico',
  ...
  await must('insert meeting minutes', service.from('meeting_minutes').insert({
    ...
    board_session_id: boardSession.id,
    minutes: 'Pre-minuta gerada pelo Board Brain: o board converge na tese de owned audience, mas recomenda commit with conditions com gates financeiros, operacionais, reputacionais e de dados.',
    decisions_presented: decisionCandidates,
    conflicts_identified: [
      'Growth quer acelerar monetizacao via app/CRM; Risk exige guardrails de dados, betting e editorial.',
  ...
  const firstDecisionId = decisions[0]?.id ?? null
  await must('insert follow ups', service.from('follow_ups').insert([
    ...
    },
    {
      ...
      company_id: company.id,
      governance_cycle_id: governanceCycle.id,
      decision_id: decisions[1]?.id ?? null,
  ...
  await must('insert compatibility governance run', service.from('governance_runs').insert({
    ...
    raw_output: {
      ...
      follow_ups_created: 4,
    },
    model_provider: 'research_seed',
    model_name: 'official-media-kit-plus-board-os-analysis',
    risk_score: 70,
    confidence_score: 79,
    executive_summary: diagnosis,
    board_pack: {
    ...
    governance_score: {
      total: 84,
      explanation: 'Forte base publica de audiencia e produto; lacunas financeiras e operacionais internas limitam confianca.',
    },
    meeting_agenda: [
      'App/CRM first-party',
      'DRE gerencial',
...
async function main() {
  ...
  const sourceDocumentId = await uploadMediaKit(organization, company, user)
  await must('insert company brain entries', service.from('company_brain_entries').insert(brainEntries(organization, company, user, sourceDocumentId)))
  const chain = await seedGovernanceChain(organization, company, user, sourceDocumentId)
